...
export default function Feed({ publicaciones: propsPublicaciones }) {
  ...
  const [datosCompletamenteCargados, setDatosCompletamenteCargados] =
    useState(false);
  ...
  const cargarReaccionesComentarios = useCallback(async () => {
    try {
      ...
      const puntuacionesComentariosResponse = await axios.get(
        "/api/comentarios/puntuaciones",
        {
          withCredentials: true,
          headers: {
            "Cache-Control": "no-cache, no-store, must-revalidate",
      ...
      if (reaccionesComentariosResponse.data) {
        setReaccionesComentarios((prevReacciones) => {
          ...
          Object.entries(reaccionesComentariosResponse.data).forEach(
            ([id, valor]) => {
      ...
      if (puntuacionesComentariosResponse.data) {
        setComentarios((prevComentarios) => {
          ...
          Object.keys(comentariosActualizados).forEach((publicacionId) => {
            if (
              comentariosActualizados[publicacionId] &&
              comentariosActualizados[publicacionId].length > 0
            ) {
              comentariosActualizados[publicacionId] = comentariosActualizados[
                publicacionId
              ].map((comentario) => {
                if (
                  puntuacionesComentariosResponse.data[comentario.id] !==
                  undefined
                ) {
                  return {
                    ...comentario,
      ...
      console.error(
        "Error al cargar reacciones o puntuaciones de comentarios:",
        e
      );
    }
  }, []);
  ...
  const fetchConteoRespuestas = useCallback(async (comentarioId) => {
    try {
      const response = await axios.get(
        `/api/comentario/${comentarioId}/respuestas/conteo`,
        { withCredentials: true }
      );
      ...
      console.error(
        `Error al obtener conteo de respuestas para comentario ${comentarioId}:`,
        error
      );
    }
  }, []);
  ...
  const fetchComentarios = useCallback(
    async (publicacionId) => {
      try {
        ...
        console.error(
          `Error al cargar comentarios de la publicación ${publicacionId}:`,
          error
        );
      } finally {
        setCargandoComentarios((prev) => ({
    ...
    try {
      const conteosResponses = await Promise.all(
        publicaciones.map((publi) =>
          axios.get(`/api/publicacion/${publi.id}/comentarios/conteo`, {
            withCredentials: true,
          })
        )
      );
  ...
  const likeComentario = async (comentarioId, publicacionId) => {
    ...
    setComentarios((prev) => ({
      ...
      [publicacionId]: prev[publicacionId].map((c) =>
        c.id === comentarioId
          ? { ...c, puntuacion: (c.puntuacion ?? 0) + ajuste }
          : c
      ),
    }));
    ...
    setRespuestas((prev) => {
      ...
      Object.keys(updated).forEach((parentId) => {
        updated[parentId] = updated[parentId].map((r) =>
          r.id === comentarioId
            ? { ...r, puntuacion: (r.puntuacion ?? 0) + ajuste }
            : r
        );
      });
      return updated;
    ...
    setReaccionesComentarios((prev) => ({
      ...prev,
      [comentarioId]: nuevaReaccion,
    }));

    try {
      if (nuevaReaccion === null) {
        // quitar reacción
        await axios.delete(`/api/reaccion-comentario/${comentarioId}`, {
          withCredentials: true,
        });
      } else {
        // poner like
        await axios.post(
          `/api/reaccion-comentario/${comentarioId}/like`,
          {},
          { withCredentials: true }
        );
      }
      ...
      console.error(
        `Error al cambiar reacción al comentario ${comentarioId}:`,
        e
      );
      setComentarios((prev) => ({
        ...prev,
  ...
  const dislikeComentario = async (comentarioId, publicacionId) => {
    ...
    try {
      if (nuevaReaccion === null) {
        ...
        await axios.post(
          `/api/reaccion-comentario/${comentarioId}/dislike`,
          {},
          { withCredentials: true }
        );
      }
  ...
  const toggleComentarios = async (publicacionId) => {
    ...
    if (
      ...
      comentarios[publicacionId].length === 0
    ) {
  ...
  const obtenerComentarioPadre = useCallback(
    (comentarioId, publicacionId) => {
      const comentariosPublicacion = comentarios[publicacionId] || [];
      const comentario = comentariosPublicacion.find(
        (c) => c.id === comentarioId
      );
  ...
  const handleComentar = async (
    ...
    let texto = respuestaA
      ? contenidoRespuesta[respuestaA]
      : contenidoComentario[publicacionId];
    ...
    if (
      respuestaA &&
      usuarioNombre &&
      !textoFinal.startsWith(`@${usuarioNombre} `)
    ) {
      textoFinal = `@${usuarioNombre} ` + textoFinal;
    }

    const comentarioPadreId = respuestaA
      ? obtenerComentarioPadre(respuestaA, publicacionId)
      : null;
    ...
    const nuevoComentario = {
      ...
      respondidoUsuario:
        respuestaA !== comentarioPadreId ? usuarioNombre : null,
      _esNuevo: true,
    };
    ...
    try {
      ...
      if (response.data && response.data.id) {
        setComentarios((prev) => ({
          ...prev,
          [publicacionId]: prev[publicacionId].map((c) =>
            c.id === nuevoComentarioId
              ? { ...c, id: response.data.id, _esNuevo: false }
              : c
          ),
        }));
        ...
        if (respuestaA) {
          setRespuestas((prev) => ({
            ...
            [comentarioPadreId]: prev[comentarioPadreId].map((r) =>
              r.id === nuevoComentarioId
                ? { ...r, id: response.data.id, _esNuevo: false }
                : r
            ),
          }));
        }
      ...
      const conteoResponse = await axios.get(
        `/api/publicacion/${publicacionId}/comentarios/conteo`,
        { withCredentials: true }
      );
      ...
      if (respuestaA) {
        const conteoRespuestasResponse = await axios.get(
          `/api/comentario/${comentarioPadreId}/respuestas/conteo`,
          { withCredentials: true }
        );
        if (
          conteoRespuestasResponse.data &&
          conteoRespuestasResponse.data.count !== undefined
        ) {
          setConteosRespuestas((prev) => ({
            ...prev,
      ...
      setComentarios((prev) => ({
        ...prev,
        [publicacionId]: prev[publicacionId].filter(
          (c) => c.id !== nuevoComentarioId
        ),
      }));
      setConteosComentarios((prev) => ({
        ...prev,
            ...
            >
              ...
              <div className="d-flex gap-2">
                <Button
                  variant="success"
                  size="sm"
                  onClick={() => toggleCajaComentar(publi.id)}
                >
              ...
              {comentariosVisibles[publi.id] && (
                <div className="mt-3">
                  ...
                  ) : comentarios[publi.id] &&
                    comentarios[publi.id].length > 0 ? (
                    comentarios[publi.id]
                      .filter((c) => !c.respuestaA)
                      .map((c) => (
                        <div
                          key={c.id}
                        ...
                        >
                          ...
                          <div className="d-flex gap-2 align-items-center mt-1">
                            <Button
                              size="sm"
                              variant={
                                reaccionesComentarios[c.id] === "like"
                                  ? "primary"
                                  : "outline-primary"
                              }
                            ...
                            <Button
                              size="sm"
                              variant={
                                reaccionesComentarios[c.id] === "dislike"
                                  ? "danger"
                                  : "outline-danger"
                              }
                            ...
                            <span>
                              <strong>{c.puntuacion ?? 0}</strong>
                            </span>
                            <Button
                              size="sm"
                              variant="outline-primary"
                              onClick={() => toggleCajaResponder(c.id)}
                            >
                              {mostrarCajaResponder[c.id]
                                ? "Cancelar"
                                : "Responder"}
                            </Button>
                            {(conteosRespuestas[c.id] === undefined ||
                              conteosRespuestas[c.id] > 0) && (
                              <Button
                                size="sm"
                                variant="outline-secondary"
                                onClick={() => toggleRespuestas(c.id)}
                              >
                                {respuestasVisibles[c.id]
                                  ? "Ocultar respuestas"
                                  : `Mostrar respuestas (${
                                      conteosRespuestas[c.id] ?? "..."
                                    })`}
                              </Button>
                            )}
                          </div>
                          ...
                          {respuestasVisibles[c.id] &&
                            (c.id in cargandoRespuestas &&
                            cargandoRespuestas[c.id] ? (
                              ...
                              respuestas[c.id]?.map((r) => {
                                ...
                                return (
                                  <div
                                    key={r.id}
                                    className="respuesta p-2 mt-2 border rounded"
                                  >
                                    {formatearContenidoComentario(
                                    ...
                                    <div className="d-flex gap-2 align-items-center mt-1">
                                      <Button
                                        size="sm"
                                        variant={
                                          reaccionesComentarios[r.id] === "like"
                                            ? "primary"
                                            : "outline-primary"
                                        }
                                        onClick={() =>
                                          likeComentario(r.id, publi.id)
                                        }
                                      >
                                        👍
                                      </Button>
                                      <Button
                                        size="sm"
                                        variant={
                                          reaccionesComentarios[r.id] ===
                                          "dislike"
                                            ? "danger"
                                            : "outline-danger"
                                        }
                                        onClick={() =>
                                          dislikeComentario(r.id, publi.id)
                                        }
                                      >
                                        👎
                                      </Button>
                                      <span>
                                        <strong>
                                          {comentarioActualizado.puntuacion ??
                                            0}
                                        </strong>
                                      </span>
                                      <Button
