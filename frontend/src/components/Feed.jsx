import React, { useEffect, useState, useCallback } from 'react';
  ...
  export default function Feed() {
    ...
    const fetchComentarios = useCallback(async (publicacionId) => {
      try {
        ...
        .filter(c => c.respuestaA === null) // solo los comentarios raíz
        .forEach(c => {
    ...
    const fetchConteosComentarios = useCallback(async () => {
      try {
        // Obtener conteos para todas las publicaciones
        ...
        // Procesar respuestas y actualizar el estado
        const nuevosConteos = {};
        conteosResponses.forEach((response, index) => {
          const publicacionId = publicaciones[index].id;
          // Extraer el número del campo "count" de la respuesta
        ...
        // Marcar que los datos están completamente cargados
        setDatosCompletamenteCargados(true);
      } catch (error) {
        console.error('Error al cargar conteos de comentarios:', error);
        // Incluso en caso de error, marcamos como completado para mostrar la UI
        setDatosCompletamenteCargados(true);
      }
    }, [publicaciones]);
    ...
    const dislikeComentario = async (comentarioId, publicacionId) => {
      ...
      if (reaccionActual === 'dislike') {
          ...
          ajuste = -2; // cambiar like → dislike
      } else {
          ajuste = -1; // poner dislike nuevo
      }
    ...
    const toggleComentarios = async (publicacionId) => {
      // Si ya están visibles, solo cambiar el estado y salir
      if (comentariosVisibles[publicacionId]) {
        setComentariosVisibles(prev => ({ ...prev, [publicacionId]: false }));
        return;
      }
      
      // Marcar los comentarios como visibles inmediatamente para que el botón cambie
      ...
      if (!comentarios[publicacionId] || comentarios[publicacionId].length === 0) {
        // La función fetchComentarios ya maneja el estado de carga
    ...
    const toggleCajaComentar = async (publicacionId) => {
      ...
      if (isOpen) {
          ...
          // Si al abrir la caja aún no están cargados los comentarios, los cargamos
          if (!comentarios[publicacionId] || comentarios[publicacionId].length === 0) {
              await fetchComentarios(publicacionId);
          }
      }
  ...
  const toggleCajaResponder = async (comentarioId) => {
    ...
    if (isOpen) {
        ...
        // Si al abrir la caja aún no están cargadas las respuestas, las cargamos
